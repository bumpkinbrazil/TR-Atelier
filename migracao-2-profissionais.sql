-- ============================================================
-- MIGRACAO 2 — Profissionais + bloqueio de agenda + link de cancelamento
...
create table if not exists public.professionals (
  ...
  role text not null default '',           -- ex.: "Cabeleireiro(a)", "Barbeiro"
  active boolean not null default true,
  sort_order int not null default 0,
  created_at timestamptz not null default now()
);

-- ---------- Bloqueios de agenda (folgas, feriados, almoco) ----------
create table if not exists public.blocked_slots (
  id uuid primary key default gen_random_uuid(),
  professional_id uuid references public.professionals(id) on delete cascade, -- null = todos
  block_date date not null,
  block_time time,                          -- null = dia inteiro
...
-- ---------- Seed: 1 profissional padrao + associa agendamentos antigos ----------
insert into public.professionals (name, role, sort_order)
select 'Profissional 1', 'Cabeleireiro(a)', 1
where not exists (select 1 from public.professionals);

update public.appointments
set professional_id = (select id from public.professionals order by sort_order limit 1)
where professional_id is null;

-- ============================================================
-- FUNCOES (dropar as antigas e recriar com suporte a profissional)
-- ============================================================
...
-- Horarios livres considerando o profissional (ou "sem preferencia" = qualquer um).
...
begin
  ...
  while t + v_duration <= v_close_min loop
    if t > v_now_min then
      -- quantos profissionais estao livres nesse intervalo
      select count(*) into v_free_pros
      from public.professionals p
      where p.active = true
        and (p_professional_id is null or p.id = p_professional_id)
        -- sem agendamento sobreposto desse profissional
        and not exists (
        ...
        -- sem bloqueio (do proprio pro ou global): dia inteiro ou horario dentro do intervalo
        and not exists (
...
-- Cria agendamento; se p_professional_id for null, escolhe automaticamente um livre.
...
begin
  ...
  where p.active = true
    ...
    and not exists (
      ...
      where a.professional_id = p.id and a.appt_date = p_date and a.status <> 'cancelado'
...
-- Cliente cancela pelo link (id + token), sem login.
...
begin
  select * into v_row from public.appointments where id = p_id and manage_token = p_token;
  if not found then return json_build_object('ok',false,'error','Agendamento nao encontrado.'); end if;
  if v_row.status = 'cancelado' then return json_build_object('ok',true,'already',true); end if;
  if v_row.appt_date < (now() at time zone 'America/Sao_Paulo')::date then
    return json_build_object('ok',false,'error','Este agendamento ja passou.');
  end if;
...
-- Consulta os dados de um agendamento pelo link (para a pagina de gerenciamento).
create or replace function public.get_appointment(p_id uuid, p_token uuid)
returns json
language plpgsql
stable
security definer
...
begin
  select json_build_object(
    'ok', true,
    'id', a.id, 'status', a.status, 'date', a.appt_date, 'time', to_char(a.appt_time,'HH24:MI'),
    'name', a.customer_name, 'service', s.name, 'professional', coalesce(p.name,''), 'duration', s.duration_min
  ) into v_j
  from public.appointments a
  join public.services s on s.id = a.service_id
  left join public.professionals p on p.id = a.professional_id
  where a.id = p_id and a.manage_token = p_token;
  if v_j is null then return json_build_object('ok', false, 'error', 'Agendamento nao encontrado.'); end if;
  return v_j;
end;
$$;

-- ============================================================
-- SEGURANCA (RLS) das tabelas novas
-- ============================================================
...
create policy blocks_admin on public.blocked_slots
  for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
grant select on public.blocked_slots to anon;  -- get_available_slots roda como definer; leitura direta nao e' necessaria, mas nao expoe dado sensivel
...
revoke all on function public.cancel_appointment(uuid, uuid) from public;
grant execute on function public.cancel_appointment(uuid, uuid) to anon, authenticated;

revoke all on function public.get_appointment(uuid, uuid) from public;
grant execute on function public.get_appointment(uuid, uuid) to anon, authenticated;

-- ============================================================
-- FIM. Agora o sistema tem profissionais, bloqueios de agenda e
-- link de cancelamento. Cadastre seus profissionais no painel.
-- ============================================================
